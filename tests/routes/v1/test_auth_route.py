...
async def test_get_token_should_return_200_OK_POST(client, session, normal_user):
    response = await client.post(
        f"{base_auth_route}/sign-in", json={"email__eq": normal_user["email"], "password": normal_user["password"]}
    )
...
async def test_auth_token_wrong_user_should_return_401_INVALIDCREDENTIALS_POST(client, session, normal_user):
    response = await client.post(
        f"{base_auth_route}/sign-in", json={"email__eq": normal_user["email"], "password": "wrong_password"}
    )
...
@pytest.mark.anyio
async def test_auth_get_me_should_return_200_OK_GET(client, session, normal_user_token):
